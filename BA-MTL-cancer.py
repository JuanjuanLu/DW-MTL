...
args['task_name'] = 'DWMT_cancer'  # change
args['data_name'] = 'DWMT_cancer'  # change
args['times'] = 10

# selected task, generate select task index, task class, and classification_num
args['select_task_list'] = ['ISF', 'IUR', 'OSF', 'APO', 'ELM', 'PRO', 'EPM', 'GIN', 'OXP']  # change
args['select_task_index'] = []
args['classification_num'] = 0
args['regression_num'] = 0
args['all_task_list'] = ['ISF', 'IUR', 'OSF', 'APO', 'ELM', 'PRO', 'EPM', 'GIN', 'OXP']  # change
...
for task in args['select_task_list']:
    if task in ['APO', 'ELM', 'PRO', 'EPM', 'GIN', 'OXP']:
        args['classification_num'] = args['classification_num'] + 1
    if task in ['ISF', 'IUR', 'OSF']:
...
try:
    test_func = run_an_eval_epoch_heterogeneous
    print("run_an_eval_epoch_heterogeneous")
except NameError as e:
    print(f"{e}")
    print("utils.MY_GNN...")
    from utils.MY_GNN import run_an_eval_epoch_heterogeneous
    print("run_an_eval_epoch_heterogeneous")
...
for time_id in range(args['times']):
    ...
    current_task_name = f"{args['task_name']}_time{time_id+1}"
    stopper = EarlyStopping(patience=args['patience'], task_name=current_task_name, mode=args['mode'])
    ...
    model_save_path = os.path.join(model_save_dir, f"{current_task_name}_best_model.pth")
    torch.save({
...
model_info_df = pd.DataFrame({
    'time_id': range(args['times']),
    'model_file': [f"{args['task_name']}_time{i+1}_best_model.pth" for i in range(args['times'])]
})
model_info_df.to_csv(os.path.join(model_save_dir, f"{args['task_name']}_models_summary.csv"), index=False)
